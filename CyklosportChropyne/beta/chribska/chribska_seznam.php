...
<body class="nav-is-fixed chribska" style=""> <!-- nav-is-fixed background-image:url('../images/cyklo_masakr.jpg'); -->
			...
			<li class="menuChribska has-children">
				...
				<ul class="menuSecondLayerChribska hidden-submenu cd-secondary-nav is-hidden">
					...
					<li class="submenuPrihlaska">
						<a href="chribska_prihlaska.php">Přihláška</a>
					</li>
					<li class="submenuSeznam">
						<a style="font-weight: bold" href="chribska_seznam.php">
							Seznam přihlášených
						</a>
					</li>
	...
	<section id="section2" class="content cd-main-content">
		<div class="aktualita">
			...
			<div class="textPropozic" style="max-width: 1200px;">
			<!--<h3>
				Další aktualizace se již do tabulky neprojeví.
			</h3> --><br>

					<?php
					include 'db_active.php';
					$connection->set_charset("utf8");
					$sql2 = "SELECT id, start, prijmeni, jmeno, obec, rok_narozeni, kategorie, tym, zaplaceno, trasa, firemni FROM zavodnici2018 ORDER BY `zavodnici2018`.`prijmeni` ASC";
					...
					if ($result2->num_rows > 0) {
						...
						while ($row = $result2->fetch_assoc()) {
							if ($row["trasa"] == "1") {
								...
								if ($row["zaplaceno"] == 1) {
									$zaplaceno = "ANO";	
								} elseif ($row["zaplaceno"] == 2) {
									$zaplaceno = "ANO <br><span class='bez_darku'>bez dárku</span>";	
								}elseif ($row["zaplaceno"] == 3) {
									$zaplaceno = "SK <br><span class='bez_darku'>s dárkem</span>";	
								}elseif ($row["zaplaceno"] == 4) {
									$zaplaceno = "SK <br><span class='bez_darku'>bez dárku</span>";	
								} else {
									$zaplaceno = "NE";
								}
								$dlouhaTrasa .= "<tr><td>".$row["start"]."</td><td>".$row["prijmeni"]."</td><td>".$row["jmeno"]."</td><td>".$row["obec"]."</td><td>".$row["rok_narozeni"]."</td><td>".$row["kategorie"]."</td><td>".$row["tym"]."</td><td>".$zaplaceno."</td><td>".$row["firemni"]."</td></tr>";
								...
								$kratkaTrasa .= "<tr><td>".$row["start"]."</td><td>".$row["prijmeni"]."</td><td>".$row["jmeno"]."</td><td>".$row["obec"]."</td><td>".$row["rok_narozeni"]."</td><td>".$row["kategorie"]."</td><td>".$row["tym"]."</td><td>".$zaplaceno."</td><td>".$row["firemni"]."</td></tr>";

							} else {

							}
						}
						echo "<h1 class='trasa-nadpis' style='color:black;'>Dlouhá<b>trasa</b> - <b>T50</b></h1>";
						echo "<table class='seznamPrihlasenych' rules='rows'> <thead><tr><td>St.č.</td> <td>Příjmení</td> <td>Jméno</td><td>Bydliště</td><td>Rok nar.</td><td>Kategorie</td><td>Tým</td><td>Zaplaceno</td><td>Firemní tým</td></tr></thead><tbody>";
						echo $dlouhaTrasa;
						echo "</tbody></table>";
						echo "<br><h1 class='trasa-nadpis' style='color:black;'>Krátká<b>trasa</b> - <b>T30</b></h1>";
						echo "<table class='seznamPrihlasenych' rules='rows'> <thead><tr><td>St.č.</td> <td>Příjmení</td> <td>Jméno</td><td>Bydliště</td><td>Rok nar.</td><td>Kategorie</td><td>Tým</td><td>Zaplaceno</td><td>Firemní tým</td></tr></thead><tbody>";
  						echo $kratkaTrasa;
  						echo "</tbody></table>"; 
	...
	<div class="footer" style="position:fixed;">
		...
		<div class="reklamy chribskaSponzori" style="text-align:center; position:absolute; top:0;left:0; width:100%; height:100%;">
			<a id="f1" href="http://www.mesto-kromeriz.cz" class="displayed" target="_blank">
				<img class="obrazky-sponzori levaobrazky-sponzori" src="../images/sponsors/km-white.png"></a>
			<a id="s1" href="http://www.force.cz" class="displayed" target="_blank">
				<img class="obrazky-sponzori stredníobrazky-sponzori" src="../images/sponsors/logo-force-white.png" style="max-width:25%;"></a>
			<a id="t1" href="http://www.elmo.cz/en/" class="displayed" target="_blank">
				<img class="obrazky-sponzori pravaobrazky-sponzori" src="../images/loga/elmo_Logo.png" class="demaLogo"></a>
			<a id="f2" href="http://www.kruzik.cz" class="hidden" target="_blank">
				<img class="obrazky-sponzori levaobrazky-sponzori" src="../images/loga/kruzik_Logo.png"></a>
			<a id="s2" href="http://www.force.cz" class="hidden" target="_blank">
				<img class="obrazky-sponzori stredniobrazky-sponzori" src="../images/sponsors/logo-force-white.png" style="max-width:25%;"></a>
			<a id="t2" href="http://www.kr-zlinsky.cz" class="hidden" target="_blank">
				<img class="obrazky-sponzori pravaobrazky-sponzori" src="../images/loga/zl_kraj_Logo.png"></a>
